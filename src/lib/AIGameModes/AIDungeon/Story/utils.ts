...
export function cutTrailingAction(text: string): string {
    ...
    let lastLine = lastPara[lastPara.length - 1].trim();

    if (
        (lastLine.toLowerCase().includes("you ask") || lastLine.toLowerCase().includes("you say")) &&
        lines.length > 1
    ) {
...
export function cutTrailingSentence(text: string): string {
    ...
    let lastPunc = Math.max(text.lastIndexOf("."), text.lastIndexOf("!"), text.lastIndexOf("?"));

    if (lastPunc <= 0) {
        lastPunc = text.length - 1;
    }

    const etToken = text.indexOf("<");
    if (etToken > 0) {
    ...
    text = fixTrailingQuotes(text);
    text = cutTrailingAction(text);
    return text;
}
/**
...
export function replaceOutsideQuotes(text: string, currentWord: string, replWord: string): string {
    ...
    const regExpr = new RegExp(currentWord + '(?=([^"]*"[^"]*")*[^"]*$)', 'g');

    return text.replace(regExpr, replWord);
}
...
export function isFirstPerson(text: string): boolean {
    ...
    for (const pair of firstToSecondMappings) {
        ...
        for (const variation of variations) {
            const regExpr = new RegExp(variation[0] + '(?=([^"]*"[^"]*")*[^"]*$)', 'g');
...
export function firstToSecondPerson(text: string): string {
    ...
    for (const pair of firstToSecondMappings) {
        const variations = mappingVariationPairs(pair);
        for (const variation of variations) {
            text = replaceOutsideQuotes(text, variation[0], variation[1]);
        }
    }

    return capitalizeFirstLetters(text.trim());
}
