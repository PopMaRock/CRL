...
>
	...
	>
		...
		<!-- --- / --- -->
		<AppRailTile on:click={()=>open=true} bind:group={currentRailCategory} name="worlds" title="Worlds" value={'/3'}>
			<svelte:fragment slot="lead"><GameIconsWorld class="inline-flex text-2xl" /></svelte:fragment>
		</AppRailTile>
		<AppRailTile on:click={()=>open=true} bind:group={currentRailCategory} name="chronicles" value={'/4'}>
			<svelte:fragment slot="lead"
				><GameIconsTiedScroll class="inline-flex text-2xl" /></svelte:fragment
			>
		</AppRailTile>
		<hr class="opacity-30" />
		<AppRailTile on:click={()=>open=true} bind:group={currentRailCategory} name="bugs" value={'/5'}>
			<svelte:fragment slot="lead"
				><GameIconsSpottedBug class="inline-flex text-xl" /></svelte:fragment
			>
		</AppRailTile>
		<AppRailTile on:click={()=>open=true} bind:group={currentRailCategory} name="information" value={'/6'}>
			<svelte:fragment slot="lead"><GameIconsHelp class="inline-flex text-xl" /></svelte:fragment>
		</AppRailTile>
		<svelte:fragment slot="trail">
			<AppRailAnchor on:click={()=>open=true} href="/" target="_blank" title="Account">
				<svelte:fragment slot="lead">
...
{#if open}
	<section class="space-y-4 overflow-y-auto p-4 pb-20 w-[30vh]" transition:logicalPropertiesHorizontalSlide={{ direction: 'inline', duration: 100 }}>
		...
		{#if currentRailCategory == "/1"}
			<a href='/GameModes/AIDungeon2/'>AIDungeon2</a><br/>
			<a href='/GameModes/ClassicRPG/'>Classic RPG</a>
			<hr class="opacity-30 mt-20" />
			<div class="text-primary-800 font-semibold mt-2">Your Adventures</div>
